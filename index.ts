...
const ScanContractSchema = z.object({
  ...
  apiToken: z.string().optional().describe("SolidityScan API token (optional if set in environment)"),
...
class SolidityScanMCPServer {
        ...
        tools: [
          {
            ...
            inputSchema: {
              ...
              properties: {
                ...
                apiToken: {
                  type: "string",
                  description: "SolidityScan API token (optional if set in environment)",
                },
              },
  ...
  private async scanAndGetReportPDF(args: any): Promise<CallToolResult> {
  ...
  private getApiToken(providedToken?: string): string {
    ...
    if (!token) {
      throw new Error("No API token provided. Please set SOLIDITYSCAN_API_KEY environment variable or provide token as parameter.");
    }
    return token;
  }

  private async scanContract(args: any): Promise<CallToolResult> {
  ...
  private async scanProject(args: any): Promise<CallToolResult> {
  ...
  private async loadPlatformChain() {
    ...
    try {
      ...
      const dataByPlatformName: Record<string, { id: string; chainsByName: Record<string, string | null> }> = {};
      const dataByPlatformId: Record<string, string> = {};
      ...
      const empty = { platforms: [], chains: [], dataByPlatformName: {}, dataByPlatformId: {} } as {
        platforms: string[];
        chains: string[];
        dataByPlatformName: Record<string, { id: string; chainsByName: Record<string, string | null> }>;
        dataByPlatformId: Record<string, string>;
  ...
  private async resolvePlatformAndChain(platformInput: string, chainInput: string): Promise<{ platformId: string; platformName: string; chainId: string | null; chainName: string }> {
    ...
    if (index.dataByPlatformName[platformInput]) {
      ...
      // Even if an ID is provided, normalize to name for usage.
  ...
  private async scanLocalDirectory(args: any): Promise<CallToolResult> {
  ...
  private async scanFileContent(args: any): Promise<CallToolResult> {
